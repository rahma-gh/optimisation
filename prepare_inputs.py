...
# Chaque entrée liste les patterns regex qui, s'ils apparaissent dans le
# code d'un test, indiquent que ce test est sensible à l'élément concerné.
# CORRECTION : ajout des champs JSON manquants pour couvrir les dépendances
# indirectes (rotation_events, return_events, sequence_complete, etc.)
SENSITIVITY_KEYWORDS = {
    "speed": [
        r"speed",
        r"vitesse_bras",
        r"wb_motor_set_velocity",
        r"rotation_events",      # AJOUT : rotation bloquée si speed=0
        r"return_events",        # AJOUT : retour bloqué si speed=0
        r"wrist_reached_target", # AJOUT : wrist jamais atteint si bras immobile
        r"distance_sensor_triggered",  # AJOUT : cycle ne reprend pas si bras bloqué
        r"gripper_opened",       # AJOUT : releasing jamais atteint si bras immobile
    ],
    "TIME_STEP": [
        r"TIME_STEP",
        r"timestep",
    ...
    "target_positions": [
        r"target_position",
        r"positions_cibles",
        r"rotation_events",      # AJOUT : rotation incomplète si position cible invalide
        r"return_events",        # AJOUT : pas de retour si RELEASING jamais atteint
    ],
    "distance_threshold": [
        r"distance_sensor",
        r"< 500",
    ...
    "gripper_position": [
        ...
        r"hand_motor",
        r"gripper_pos",
        r"gripper_closed",       # AJOUT : fermeture impactée par la valeur du gripper
    ],
    "wrist_threshold": [
        r"wrist",
        r"\-2\.3",
    ...
    "HAUTEUR_SAISIE": [
        ...
        r"grasped",
        r"grasp_event",
        r"sequence_complete",    # AJOUT : séquence jamais complète si saisie invalide
        r"distance_sensor_triggered",  # AJOUT : capteur semble inactif sans saisie validée
        r"gripper_opened",       # AJOUT : relâchement impossible sans saisie validée
        r"gripper_closed",       # AJOUT : fermeture invalidée si saisie échoue
        r"release_event",        # AJOUT : dépôt impossible sans saisie
        r"wrist_reached_target", # AJOUT : cycle complet compromis
        r"return_events",        # AJOUT : retour bloqué si cycle incomplet
        r"rotation_events",      # AJOUT : chaîne complète compromise
    ],
    "DEPLACEMENT_DEPOT": [
        r"DEPLACEMENT_DEPOT",
        r"deplacement.*depot",
    ...
    "STEPS_STABLE": [
        r"STEPS_STABLE",
        r"steps_stable",
        r"sequence_complete",       # AJOUT : timeout peut survenir avant fin si STEPS_STABLE trop grand
        r"release_event",           # AJOUT : validation du dépôt retardée
        r"return_events",           # AJOUT : chaque transition retardée
        r"rotation_events",         # AJOUT : idem
        r"grasp_event",             # AJOUT : validation saisie retardée
        r"can_final_positions",     # AJOUT : positions enregistrées au timeout → déplacements aberrants
        r"PRECISION_MAX",           # AJOUT : seuil de déplacement max impacté par timeout prématuré
        r"deplacement",             # AJOUT : calcul de distance impacté si simulation interrompue
    ],
}
